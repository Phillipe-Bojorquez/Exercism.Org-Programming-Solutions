...
export function fixListOfWagons(EachWagonsID) {
  const [a, b, ...spread] = EachWagonsID;
  const newArray = [...spread, a, b];
  return newArray;
}
...
export function correctListOfWagons(ids, missingWagons) {
  ...
  const newArray = [a, ...missingWagons, ...everythingelse]
  return newArray
}
...
export function extendRouteInformation(information, additional) {
  const consolidated = {...information, ...additional};
  return consolidated;
}
...
export function separateTimeOfArrival(information) {
  const {timeOfArrival, ...informationWithoutArrivalTime} = information;
  return [timeOfArrival, informationWithoutArrivalTime];
}
